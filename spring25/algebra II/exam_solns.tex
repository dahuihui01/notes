...
\begin{prob}[Q5]
    Let \( \xi \in \bigwedge^p V \) be a non-zero \( p \)-vector. Put \( W = \{ v \in V \mid \xi \land v = 0 \} \). Verify that \(\dim W \leq p\) and equality holds only when \( \xi \) is splitting.
\end{prob}
\begin{proof}
...
\begin{proof}
    ...
    \begin{equation*}
        E=\Q(s_1,\dots, s_n)
    \end{equation*} 
    where $s_i$ are elementary symmetric polynomials. We see that 
    \begin{equation*}
        \gal(F/E)=S_n
    \end{equation*}
    Let
...
\begin{proof}
    \begin{enumerate}
        ...
        % Suppose that $\psi=\sum_{i}a_iw_i$, where $\{w_i\}$ is basis for $\bigwedge^n(V)$. Then 
        % \item \begin{equation*}
        %     \left(\sum_ia_iw_i\right)=\sum_ia_iw_i\wedge v=0
        % \end{equation*}
        % This implies that $w_i\wedge v$ is a linearly dependent set in $\bigwedge^{n+1}(V)$. In other words, we can write 
        % \begin{equation*}
        %     w_1\wedge v=\sum_{i\geq 2}b_iw_i\wedge v
        % \end{equation*}
        % Hence we can write $\psi$ as 
        % \begin{equation*}
        %     \psi=a_i w_i
        % \end{equation*}
        ...
        % $\psi=v_1\wedge\dots\wedge v_n$, and $\psi\wedge v$ is an element in $\bigwedge^{n+1}V$, then we know 
        % \begin{equation*}
        %     v_1\wedge\dots\wedge v_n\wedge v=0\iff \{v_1,\dots, v_n, v \} \text{ linearly dependent in $V$}
        % \end{equation*}
        % Hence writing $v_1=cv+\sum_{i=2}^na_iv_i$, we have 
        % \begin{equation*}
        %     \psi=cv\wedge v_2\wedge\dots\wedge v_n=v\wedge\varphi
        % \end{equation*}
        % where $\varphi=cv_2\wedge\dots\wedge v_n$ is an element in $\bigwedge^{n-1}V$.
