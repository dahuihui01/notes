...
Next we talk about the argument principle.
\begin{prop}
    Let $f\in C^2(\bar{\Omega})$ be holomorphic in $\Omega$, nowhere zero on $\partial\Omega$, then let $\nu(f,\Omega)$ be a the number of zeros of $f$ in $\Omega$, counted with multiplicity, 
    \begin{equation*}
...
The argument principle also works for meromorphic functions.
\begin{prop}
...
\begin{prop}
    ...
    \begin{equation*}
        \Gamma(z)=\lim_{n\to\infty}n^z\frac{1\cdot 2\cdot\dots n}{z(z+1)\dots(z+n)}
    \end{equation*}
\end{prop}
...
\chapter{Conformal maps and geometrical aspects of complex function the-
ory}
