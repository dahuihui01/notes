...
\begin{prop}[Fermat's Little Theorem]
    Let $a\in\Z/p\Z$, then 
    \begin{equation*}
        a^p\equiv a
    \end{equation*}
...
\begin{thm}
    ...
    % In other words, if a field $k$ is algebraically closed, then it is infinite.
\end{thm}
...
\begin{proof}
    ...
    % ($F$ has at most $q$ elements, but $f(x)=x^q-x$ is separable because $f,f'$ are relatively prime. 
...
\begin{prop}[Frobenius reciprocity for characters]
