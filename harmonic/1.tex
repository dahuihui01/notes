...
We will address Q1.
\begin{lemma}
    $\lim_{N\to\infty}\|S_N(f)-f\|_{L^p}=0$ if and only if there exists a constant $C_p$ such that
    \begin{equation*}
        \|S_N(f)\|_{L^p}\leq C_p\|f\|_{L^p}
    \end{equation*}
\end{lemma}
\begin{proof}
    $(\Rightarrow)$ This follows from UBP.
...
\begin{equation*}
    \|S_N(f)\|_{L^p}\leq C_p\|f\|_{L^p}
\end{equation*}
but for $p=1$, we have $\|S_N(f)\|_{L^1}=L_N$ as above. 
